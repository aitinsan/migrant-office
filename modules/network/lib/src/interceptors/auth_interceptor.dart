...
abstract interface class AuthStatusDataSource {
  ...
  ///
  /// This stream should be listened from repository and controller,
...
class AuthInterceptor<T> extends QueuedInterceptor
  ...
  Future<void> onRequest(
    ...
    try {
      ...
      Logger.instance.logError(
        e,
        stackTrace,
        hint: 'AuthInterceptor.onRequest',
        fatal: true,
      );
  ...
  Future<Response<R>> _refresh<R>(Response<R> response, T token) async {
    ...
    // Retry the request
    return await retryRequest<R>(response, headers);
  }
